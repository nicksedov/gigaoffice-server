...
class HistogramProcessorService:
    """
    Service for processing histogram analysis requests with GigaChat AI.
    
    This service handles the analysis of spreadsheet data to recommend histogram
    configurations. It takes spreadsheet data with statistical metadata, sends it
    to GigaChat for analysis, and returns histogram configuration recommendations.
    ...
    def __init__(self, gigachat_service: BaseGigaChatService):
        """
        Initialize the histogram processor service.
    ...
    async def process_histogram(
        ...
        """
        Process histogram analysis request with GigaChat
        
        Args:
            query: User instruction for histogram analysis
            category: Category type (must be "data-histogram")
            spreadsheet_data: Enhanced spreadsheet data with statistical metadata
                Expected format: Dict with 'metadata', 'worksheet', 'data', 'statistics' fields
                Statistics should contain 'count', 'min', 'median', 'max', 'range' for numerical columns
        ...
        try:
            ...
            logger.info(f"Sending histogram analysis request to GigaChat: {query[:100]}...")
            ...
            # Parse the response as HistogramResponse
            result_data = response_parser.parse_histogram_data(response_content)
            
            if result_data is None:
                logger.warning("Failed to parse histogram configuration from response")
            ...
            logger.info(
                f"Histogram processing completed successfully in {processing_time:.2f}s, "
            ...
            logger.error(f"Error processing histogram data: {e}")
            raise
